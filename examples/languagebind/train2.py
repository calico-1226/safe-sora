...
class HelpfulnessDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer):
        ...
        if is_main_process():
            print('-' * 50, ' Data Example ', '-' * 50)
            print('Prompt:', self.tokenizer.decode(self.input_ids[random_idx]))
            print('Video1:', self.video_0[random_idx])
            print('Video2:', self.video_1[random_idx])
            print('Preferred:', self.preference[random_idx])
            print()
        dist.barrier()
    ...
    def __getitem__(self, index: int):
        video = {
            'video_0': load_video_from_path(self.video_0[index]),
            'video_1': load_video_from_path(self.video_1[index]),
        }
...
def main():
    ...
    bar.close()
    if is_main_process():
        model.module.save('./outputs/test1/')
        import matplotlib.pyplot as plt

        plt.plot(loss_list)
        plt.savefig('./outputs/test1/loss.png')
        plt.close()
